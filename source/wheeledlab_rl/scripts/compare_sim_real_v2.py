...
def resample_1d(t_src, y_src, t_new):
    ...
    # drop duplicate time stamps to keep np.interp happy
...
##############################################################
    ...
    # --------------------------
    angle_diff = real_init_angle - sim_init_angle + ROTATE_TRAJ_ANGLE
    ...
    ax1b.set_ylim(-0.5, 0.5)   # <-- fix steering range
    ...
    ax3.set_ylim(-3, 3)   # <-- fix steering range
    ...
    # Command velocity
    ax1.plot(sim_time,
            sim_speed_cmd * CONFIG['env_config']['MAX_SPEED_SCALING'],
            color='green', label='Sim cmd velocity')
    ax1.plot(real_time,
            real_data['cmd_velocity'].values[real_mask],
            color='green', linestyle='--', label='Real cmd velocity')

    # Actual velocity (vx)
    ax1.plot(sim_time, sim_vel_x,
            color='blue', label='Sim vx')
    ax1.plot(real_time, real_vx,
            color='blue', linestyle='--', label='Real vx')
    ...
    ax2.plot(sim_time,
            sim_steering * CONFIG['env_config']['MAX_STEERING_SCALING'],
            color='red', label='Sim cmd steering')
    ax2.plot(real_time,
            real_data['cmd_steering_angle'].values[real_mask],
            color='red', linestyle='--', label='Real cmd steering')

    # Yaw rate (ω)
    ax2b = ax2.twinx()
    ax2b.plot(sim_time, sim_yaw_rate,
            color='purple', label='Sim ω')
    ax2b.plot(real_time,
            real_data['omega'].values[real_mask],
            color='purple', linestyle='--', label='Real ω')
    ...
    # --- Compute slip angle ---
    sim_slip_angle = np.arctan2(sim_vel_y, sim_vel_x)
    real_slip_angle = np.arctan2(real_vy, real_vx)

    # --- Create figure with 3 subplots ---
    fig, axes = plt.subplots(3, 1, figsize=(12, 10), sharex=True)

    # -----------------------------
    # Subplot 1: Velocity command vs actual velocity
    # -----------------------------
    ax1 = axes[0]
    ax1.plot(sim_time, sim_speed_cmd * CONFIG['env_config']['MAX_SPEED_SCALING'],
            color='green', label='Sim cmd velocity')
    ...
    ax1.plot(sim_time, sim_vel_x, color='blue', label='Sim v_x')
    ax1.plot(real_time, real_vx, color='blue', linestyle='--', label='Real v_x')

    ax1.set_ylabel("Velocity [m/s]")
    ax1.set_title("Velocity Command vs Actual Velocity")
    ax1.grid(True)
    ax1.legend()
    ...
    # Steering commands
    ax2.plot(sim_time, sim_steering * CONFIG['env_config']['MAX_STEERING_SCALING'],
            color='blue', label='Sim cmd steering')
    ax2.plot(real_time, real_data['cmd_steering_angle'].values[real_mask],
            color='blue', linestyle='--', label='Real cmd steering')

    # Yaw rates
    ax2b.plot(sim_time, sim_yaw_rate, color='red', label='Sim yaw rate (ω)')
    ax2b.plot(real_time, real_data['omega'].values[real_mask],
            color='red', linestyle='--', label='Real yaw rate (ω)')

    ax2.set_ylabel("Steering Angle [rad]", color='blue')
    ax2.set_ylim(-0.5, 0.5)
    ax2b.set_ylabel("Yaw Rate ω [rad/s]", color='red')
    ax2b.set_ylim(-3, 3)
    ...
    # -----------------------------
    # Subplot 3: Slip angle
    # -----------------------------
    ax3 = axes[2]
    ax3.plot(sim_time, sim_slip_angle, color='purple', label='Sim slip angle')
    ax3.plot(real_time, real_slip_angle, color='purple', linestyle='--', label='Real slip angle')

    ax3.set_ylabel("Slip angle β [rad]")
    ax3.set_xlabel("Time [s]")
    ax3.set_title("Slip Angle vs Time")
